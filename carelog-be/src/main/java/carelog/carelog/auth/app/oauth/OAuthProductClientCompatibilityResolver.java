...
/**
 * 기존 Kakao 요청의 clientChannel을 Carelog WEB 기본 Client로 해석하는 호환 계층이다.
...
public class OAuthProductClientCompatibilityResolver {
    ...
    public RegisteredProductClient resolve(OAuthAuthorizationCommand command) {
        String clientId = command.clientId() == null ? DEFAULT_CARELOG_WEB_CLIENT_ID : command.clientId();
        RegisteredProductClient client = productClientReader.readEnabled(clientId);

        if (client.product() != Product.CARELOG
                || client.channel() != ProductClientChannel.WEB
                || command.clientChannel() != ClientChannel.WEB) {
