...
class LegacyTokenSessionAdapterTest {
    ...
    @DisplayName("replaceForUser는 기존 사용자 토큰을 삭제한 뒤 신규 토큰을 저장하는 순서를 보존한다")
    @Test
    void replaceForUser_deletesThenSavesInOrder() {
        OffsetDateTime expiresAt = OffsetDateTime.parse("2026-08-01T00:00:00Z");

        adapter.replaceForUser(USER_ID, "new-token", expiresAt);
    ...
    void findByToken_mapsEntityToRefreshSession() {
        OffsetDateTime expiresAt = OffsetDateTime.parse("2026-08-01T00:00:00Z");
        RefreshToken entity = newRefreshToken(USER_ID, "raw-token", expiresAt);
    ...
    void rotate_updatesReloadedEntity() {
        ...
        RefreshSession session = new RefreshSession("old-token", USER_ID, oldExpiry);
    ...
    void rotate_doesNotCheckExpiryItself() {
        ...
        OffsetDateTime newExpiry = OffsetDateTime.parse("2026-08-15T00:00:00Z");
        RefreshToken alreadyExpiredEntity = newRefreshToken(USER_ID, "expired-token", expiredAt);
        RefreshSession session = new RefreshSession("expired-token", USER_ID, expiredAt);
    ...
    @DisplayName("deleteForUser는 RefreshTokenRepository.deleteByUserId에 위임한다")
    @Test
    void deleteForUser_delegatesToRepository() {
        adapter.deleteForUser(USER_ID);
    ...
    void delete_reloadsEntityThenDeletes() {
        OffsetDateTime expiresAt = OffsetDateTime.parse("2026-08-01T00:00:00Z");
        RefreshToken entity = newRefreshToken(USER_ID, "raw-token", expiresAt);
        RefreshSession session = new RefreshSession("raw-token", USER_ID, expiresAt);
        when(refreshTokenRepository.findByRefreshToken("raw-token")).thenReturn(Optional.of(entity));
