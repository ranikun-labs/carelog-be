...
class ProductClientRegistryIntegrationTest {
    ...
    @Test
    void Flyway가_초기_Carelog_WEB_MOBILE_Client를_등록하고_Enum과_enabled를_보존한다() {
        ...
        assertThat(productClientRepository.findByClientId("carelog-mobile"))
                .hasValueSatisfying(client -> {
                    assertThat(client.getProduct()).isEqualTo(Product.CARELOG);
                    assertThat(client.getChannel()).isEqualTo(ProductClientChannel.MOBILE);
                    assertThat(client.isEnabled()).isTrue();
                });
